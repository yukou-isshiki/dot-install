  ...
  #次に向けて
  for文だけでなく、while文でも書き換えを行う

  ##11月15日

  本日までの業務でリンクの差し替えを行いました。環境構築のために「circle CI」の設定を行ったりしました。今回の反省点としてはPRを二重に作ってしまい、結果として無用な混乱を招いてしまったことです。PRの管理には一層注意を払いたいと思います。最後に、この1ヶ月間自分自身色々なことを学んできました。次の1ヶ月でもっと色んなことを学んで、自分自身の成長につなげていきたいと考えています。

  ##11月16日、17日

  16日は一部語句の修正作業を行いました。ただ、github上で履歴がおかしくなってしまったので、ブランチを作り直してからrebaseの作業を行いました。先にrebaseしてconflictを起こしているファイルを1つ1つ直していくという方法もありましたが、結果的にはブランチの再生成の方が時間が短縮出来ました。

  Ruby入門は前回のループ文の復習をしつつ、while文での書き換えを行っています。現状では、while文によるループ設定は可能となっています。

  #引き続き学ぶこと
  whileを用いたループに条件をつけること

  #今後気をつけるべきこと
  実行されるコードの順番とその意味に注目すること

  ##11月18日、21日、24日

  業務としては、翻訳されていなかったファイルの依頼と、未翻訳のファイルの探し出しを行いました。探してみると多くのファイルが残っており、予想以上に大変であると思いました。

  ドットインストールは現在、#19を学習中です。#18、19ではメソッドとクラスを自分で定義することを学習しています。他言語では変数を定義することを良く行いましたが、Rubyでは、メソッドやクラスなどを定義するのが特徴なのでしょうか？

  #現在確認していること
  口頭で出てきた「インスタンスメソッド」の意味

  ##11月25日、30日、12月1日

  本日は#21の学習と関連している#18からもう一度おさらいしました。このところ、色々と新しい用語がいくつか出てきたこともあり、モヤモヤしていたところが蓄積されていました。しかし、本日は別の記事で今まで出てきた「インスタンス変数」、「クラス変数」、「クラスインスタンス変数」の違いがわかりやすく書かれていました。また、業務内では「インスタンス変数」しか基本的には使用しないとの事なので、今後3つのいずれかを用いる場面が出てきた場合には、「インスタンス変数」を重点的に使っていって使用しながら覚えていきたいと思います。また、先日、エディターの設定が初期設定のままだったため、「Tab」キーを押した際のスペース幅がずれてしまい、ローカルファイルとGitHub上のインデントが異なっている現象が発生していました。現在は、設定を変更しており、同様の問題は発生していません。  

  ##12月2日

  本日は#22の学習をしています。今回は「クラスの継承」に関する内容でした。業務でも今後様々なクラスが登場する事が予想され、そのような時に使用する事が予想されます。今回は、ドットインストールだけでなく、別サイトでも内容を確認しています。
  ...
  #確認中(書き換えの準備)
  スーパークラスのメソッド呼び出し
  引数があるスーパークラスのメソッド呼び出し  

  ##12月7日

  本日はドットインストール#22の続きを行い、その後、#23の学習を開始しました。#22の外部サイトの内容ですが、最初は難しく感じましたが、丁寧に見た時にそこまで難しい内容ではありませんでした。ただ、「引数があるスーパークラスのメソッドを呼び出す」項目が適切に記述できたかは、ちょっと自信がありません。#23ではメソッドのアクセス権についての内容でした。サーバーの構築においてメソッドのアクセス権を定めることは、非常に重要であると考えております。この内容についても、前回同様に外部サイトでもしっかり確認したいと思います。  
